...
function openQuickEditModal(event, unsched) {
	...
	if (disabled) {
		...
		buttonNoText = 'Ok';
		hideYesButtonStyle =
			'<style>.vex-dialog-buttons button[type="submit"] { display: none; }</style>';
	}
	...
	let statusSelectHTML =
		'<label for="post_status" class="screen-reader-text">Post Status</label><select name="post_status" required ' +
		disabled +
		'/>&nbsp;';
	if (moment(event.start).isAfter(getServerTime())) {
		statusSelectHTML +=
			'<option value="draft">Draft</option><option value="future">Future</option><option value="unsched">Unscheduled</option>';
	} else {
		statusSelectHTML +=
	...
	if (unsched === true) {
		statusSelectHTML = statusSelectHTML.replace(
			/value=\"unsched\"/i,
			'value="unsched" selected'
		);
	} else {
	...
	currentVex = vex.dialog.open({
		...
		input: [
			'<div class="calendario-modal" data-post-id="' +
				event.post_id +
				'" data-event-id="' +
				event._id +
				'">',
			'<label for="post_title" class="screen-reader-text"></label><input name="post_title" placeholder="Title" type="text" value="' +
				event.title +
				'" required ' +
				disabled +
				'/>',
			'<label for="post_date" class="screen-reader-text"></label><input type="date" name="post_date" placeholder="mm/dd/yyyy" value="' +
				eventStart +
				'" required ' +
				disabled +
				' />',
			...
			'<a class="post-edit-link" href="' +
				wpApiSettings.homeUrl +
				'wp-admin/post.php?post=' +
				event.post_id +
				'&action=edit">Edit Post</a>',
			'<a class="post-trash-link" href="' +
				wpApiSettings.homeUrl +
				'wp-admin/post.php?post=' +
				event.post_id +
				'&action=trash">Trash Post</a>',
			'</div>',
			'</div>',
			hideYesButtonStyle,
		].join(''),
		...
		callback: function (data) {
			if (!data) {
				//console.log('Cancelled');
			} else if (data.post_status == 'unsched') {
				// Make the post an Unscheduled Draft
				jQuery
					.ajax({
