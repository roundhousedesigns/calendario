...
function initPage() {
	...
	// Find the closest past Sunday to the start of the month
	let startDate = latestPostDate.date(1).subtract(firstPostDate.day(), 'days');
	
	$calendario.fullCalendar({
		defaultView: 'week',
		defaultDate: latestPostDate,
		...
		customButtons: {
			...
			scrollToLatest: {
				text: 'Latest Post',
				click: function () {
					scrollToDate(latestPostDate, 500);
				},
			},
			newPostButton: {
				text: 'New Draft',
		...
		dayRender: function (date, cell) {
			...
			if (date.date() == 1) {
				let fomLabel =
					'<span class="first-of-month-label month-name">' +
					date.format('YYYY') +
					' &bull; ' +
					date.format('MMMM') +
					'</span>';
		...
		drop: function (date) {
			// External event dropped ONTO calendar
		...
		eventReceive: function (event) {
			// Fired after fullCalendar.drop(). Dropping an event ONTO the calendar from an external source.
			let eventData = {
		...
		eventDragStop: function (event, jsEvent) {
			// Used for moving events OFF of the calendar
